...
console.log(v_kode+' '+v_pelapor+' '+v_pelanggan+' '+v_tgl_paket+' '+v_tgl_pelaporan+' '+kd_mhs);
			// mengirimkan data ke berkas transaksi.input.php untuk di proses
			$.post(url, {kode: v_kode, nama: v_pelapor, pilihikon: v_pelanggan, tgl_paket: v_tgl_paket, tgl_pelaporan:v_tgl_pelaporan, keterangan: v_keterangan, band: v_status, id: kd_mhs} ,function() {
				// tampilkan data yang sudah di perbaharui
				// ke dalam <div id="data"></div>
				$("#data-paket").load(main);
				$("#menu-paket").load(menu2);
