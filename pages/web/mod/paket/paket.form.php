...
<a href="javascript:void(0)" id="btn1" class="btn btn-xs btn-default">
<?php echo '<img src="assets/icon/'.$ikon.'"> ['.$ikon.'] '; ?>
Pilih Ikon Map
</a>
<div id="byjson"></div>  <!--memiakai plugin  ms-Dropdown-master (Select Dropdown)-->
<input type="text" id="pilih_ikon" class="form-control" name="pilih_ikon" value="" style="display: none;">  
    ...
    */
	...
	<div class="form-group">
		<label for="id" class="col-sm-2 control-label">Bandwidth</label>
			...
			<input type="number" id="band" placeholder="... Mbps" class="form-control" name="band" value="<?php echo $band ?>"> 
...
if (is_dir($dir)){
  if ($dh = opendir($dir)){
    ...
    echo    "{image:'assets/icon/".$image."', description:'Pin ikon peta', value:'".$image."', text:'".$image."'},";
    }
    }
    closedir($dh);
  }
...
$('#byjson').on('click',function(){
    var iko = $( this ).val();
    //var iko = $(".selected").val();
     document.getElementById("myNumber").val('0');
     $('#pilih_ikon').attr('disabled',true);
    console.info(iko); 
})
</script>   