	...
	<div class="form-group">
		<label for="id" class="col-sm-2 control-label">Pilih Teknisi</label>
			...
			<?php 
				...
				if($row_id_surat != 0) { 
					$query1 = mysql_query("SELECT tb_surat_jalan_teknisi.*, tb_karyawan.nama FROM `tb_surat_jalan_teknisi` JOIN `tb_karyawan` WHERE tb_surat_jalan_teknisi.id_karyawan=tb_karyawan.id_karyawan and tb_surat_jalan_teknisi.id_gangguan='$kd_gang'");
					$data1 = mysql_fetch_array($query1);	
					?>
					<option value="<?php echo $data1['id_karyawan'] ?>"><?php echo $data1['nama'] ?></option>
			<?php
			    } else{ 
                   echo "<option value=''> == Pilih Teknisi == </option>";
			    } 
				$querys = mysql_query("SELECT * FROM `tb_karyawan`");
				while($data3 = mysql_fetch_array($querys)) {
					$namateknisi = $data3['nama'];
					echo "<option value=".$data3['id_karyawan'].">".$namateknisi."</option>";
				}
				?>
			</select>
		</div>
