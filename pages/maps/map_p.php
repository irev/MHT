...
function carikordinat(lokasi){
    ...
    tanda = new google.maps.Marker({
        position: lokasi,
        map: peta,
        icon: 'assets/icon/repair.png'//+icon_pin //setjenis pin utama
    });
 ambildatabase(idpel);
...
function set_icon(ikon){
    if (ikon == "") {
        ...
        gambar_tanda = "assets/icon/repair.png"//+ikon;
    }
}
...
include('../../_db.php');
   ...
   $query = mysql_query("SELECT `p`.*,`p`.`nama` as nm_pelanggan ,`g`.*,`g`.keterangan as komen,`b`.*,`c`.merek,`c`.mac_address,`c`.keterangan as pr_ket, s.*, t.*, t.nama as teknisi FROM `tb_pelanggan` as `p` INNER JOIN `tb_gangguan` as `g` ON `p`.`id_pelanggan`=`g`.`id_pelanggan` INNER JOIN `tb_perangkat` as `c` ON `p`.`id_perangkat`=`c`.`id_perangkat` INNER JOIN `tb_paket` as `b` ON `p`.`id_paket`=`b`.`id_paket` INNER JOIN `tb_surat_jalan_teknisi` as `s` ON `s`.`id_gangguan`=`g`.`id_gangguan` INNER JOIN `tb_karyawan` as `t` ON `t`.`id_karyawan`=`s`.`id_karyawan` WHERE g.status_gangguan='3' GROUP BY p.id_pelanggan");
    $i = 0;
    $js = "";
    ...
    $js .= 'nama['.$i.'] = "'.$value['nm_pelanggan'].'";
            ...
            var contentString =  "<div class=\'col-md-12\'><!--div class=\'col-md-4\'>"+
                                 ...
                                 "<hr>Pelanggan<table>"+
                                       ...
                                       "<hr>TEKNISI<table>"+
                                        "<tr>"+      
                                           "<td><i class=\'fa fa-wrench\'></i> <b>Nama</b></td>"+
                                          "<td> <b> : </b> </td>"+
                                            "<td>  " + teknisi['.$i.'] + "</td>"+
                                        "</tr>"+
...
<div class="box box-primary">
                <div class="box-header">
                  <img src="assets/icon/repair.png"><h3 class="box-title">Lokasi Dalam Proses Peraikan</h3>
