...
var loadbarang = function() {
  ...
  //kalkulasi nilai otomatis  
   ...
   var hitung_harga = function(b) {
    ...
    var tot;
    if(!isNaN(har)){
      var har2 = Math.abs(har);
    console.log(har2);
      jQuery("#harga_" + idnya).val(har2);
    }else{
      jQuery("#harga_" + idnya).val(0);
    }
    ...
    $(".jumlah_" + idnya).each(function() {
      ...
      if (!isNaN(this.value) && this.value.length != 0 && this.value >= 1 && !isNaN(har)) {
        ...
        tot = $("#sum" + idnya).val(Math.abs(jum) * Math.abs(har));
  ...
  var totalbelanja =  function() {
    ...
    $('tr').each(function() {
      ...
      console.log(t +' ===== ' + to);
      $('.total-combat', this).html(' <h4><font color="red"><input style="baground:#333; text-align: right;" name="totalview" type="text" value="' + t + ',00" readonly/><input name="total" value="' + Math.abs(to) + '" type="hidden"></font></h4>');
